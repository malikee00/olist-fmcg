...
echo "[INFO] Forcing search_path=analytics..."
psql "${CONN}" -v ON_ERROR_STOP=1 -c "SET search_path TO analytics;"
...
psql "${CONN}" -v ON_ERROR_STOP=1 -c "\copy analytics.payment_mix(month,payment_type,total_payment_value,share) FROM '${PAYMENT_MIX}' WITH (FORMAT csv, HEADER true, NULL '')"

echo "[INFO] Loading Top Categories..."
psql "${CONN}" -v ON_ERROR_STOP=1 -c "\copy analytics.top_categories(month,product_category_name,revenue,orders,avg_price,avg_freight) FROM '${TOP_CATEGORIES}' WITH (FORMAT csv, HEADER true)"
